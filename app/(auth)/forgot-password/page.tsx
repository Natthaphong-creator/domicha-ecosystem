...
export default function ForgotPasswordPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (resetError) {
      setError(resetError.message || "ส่งลิงก์ตั้งรหัสผ่านใหม่ไม่สำเร็จ");
      return;
    }

    setMessage("ส่งลิงก์ตั้งรหัสผ่านใหม่แล้ว กรุณาตรวจสอบอีเมลของคุณ");
  }
      ...
      <form onSubmit={submit} className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-7 shadow-xl shadow-slate-200/60">
        <div className="mb-6 flex items-center gap-3">
          <Image
            ...
            priority
          />
          <div><h1 className="text-xl font-bold">DomiCha Portal Site</h1><p className="text-xs text-slate-400">Reset password</p></div>
        </div>

        <h2 className="text-2xl font-bold text-slate-950">ลืมรหัสผ่าน</h2>
        <p className="mt-1 text-sm text-slate-500">กรอกอีเมลบัญชีของคุณ ระบบจะส่งลิงก์สำหรับตั้งรหัสผ่านใหม่</p>
        ...
        <button disabled={loading} className="mt-6 w-full rounded-xl bg-orange-500 px-4 py-3 font-semibold text-white shadow-lg shadow-orange-500/20 disabled:opacity-60">
          {loading ? "กำลังส่งลิงก์..." : "ส่งลิงก์ตั้งรหัสใหม่"}
        </button>
