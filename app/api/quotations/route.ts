...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await auth.supabase.from("quotations").select(columns).order("created_at", { ascending: false });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error: fetchError } = await auth.supabase.from("quotations").select(columns).eq("id", quotation.id).single();
