...
function buildDocumentPayload(order: FranchiseeOrder, type: "invoice" | "receipt") {
  ...
  return {
    ...
    invoiceDueAt: order.invoice_due_at,
...
export async function deliverDocumentAutomation(order: FranchiseeOrder, type: "invoice" | "receipt"): Promise<ReceiptAutomationResult> {
  ...
  try {
    const response = await fetch(webhookUrl, {
      ...
      body: JSON.stringify(buildDocumentPayload(order, type))
    });
...
export async function deliverInvoiceAutomation(order: FranchiseeOrder): Promise<ReceiptAutomationResult> {
  return deliverDocumentAutomation(order, "invoice");
}

export async function deliverReceiptAutomation(order: FranchiseeOrder): Promise<ReceiptAutomationResult> {
  return deliverDocumentAutomation(order, "receipt");
}
